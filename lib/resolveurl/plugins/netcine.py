...
class NetcineResolver(ResolveUrl):
    ...
    def get_media_url(self, host, media_id):
        ...
        if player_link.startswith('/'):
            p = urlparse(web_url)
            player_link = '{0}://{1}{2}'.format(p.scheme, p.netloc, player_link)
        ...
        try:
            ...
            player_headers['Referer'] = '{0}://{1}/'.format(pp.scheme, pp.netloc)
            player_headers['Origin'] = '{0}://{1}'.format(pp.scheme, pp.netloc)
        except Exception:
            pass
        ...
        try:
            ...
            if n and p:
                ...
                fallback_query = '?n={0}&p={1}'.format(n, p)
                fallback_url = '{0}://{1}{2}{3}'.format(
                    parsed_player.scheme, parsed_player.netloc, fallback_path, fallback_query
                )
    ...
    def _finalize_headers(self, chosen, headers, player_link):
        ...
        try:
            pp = urlparse(player_link)
            final_headers['Referer'] = '{0}://{1}/'.format(pp.scheme, pp.netloc)
            final_headers['Origin'] = '{0}://{1}'.format(pp.scheme, pp.netloc)
        except Exception:
            pass
